...
void printInfo(uint16_t valuePotentiometer, uint16_t offset, int rawEigth,
               int binaryArray[], sensors_event_t event) {
...
void loop(void) {
  ...
  mag.getEvent(&event);
  event.magnetic.x = event.magnetic.x - 47;
  event.magnetic.y = event.magnetic.y + 21;
